...
interface CreateTimesheetFormData {
    user_id: string;
    project_id: string;
    task_id: string;
    date: string;
...
export default function Create({ users, projects, hasHRM, hasTaskly, onSuccess }: CreateTimesheetProps) {
    ...
    const setDataWrapper = (key: string, value: any) => {
        setData(key as keyof CreateTimesheetFormData, value);
    };

    const formFields = useFormFields('createTasklyProjectTaskField', data, setDataWrapper, {}, 'create');
        ...
        try {
            ...
            if (attendanceData && !attendanceData.error) {
                ...
                if (data.type === 'clock_in_out') {
                    ...
                    setData('minutes', attendanceData.remaining_minutes || 0);
                }
            }
        } catch (error) {
    ...
    // Update hours when type changes to clock_in_out
    useEffect(() => {
        if (data.type === 'clock_in_out' && remainingHours) {
            setData('hours', remainingHours.hours);
            setData('minutes', remainingHours.minutes);
        }
    }, [data.type, remainingHours]);
    ...
    return (
        <DialogContent className="max-w-lg">
            <DialogHeader>
                <DialogTitle>{t('Create Timesheet')}</DialogTitle>
            </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <Input
                            ...
                            type="number"
                            min="0"
                            max={data.type === 'clock_in_out' ? remainingHours.hours : 12}
                            value={data.hours}
